...
parser.add_argument('--gt_folder', default='/data1/wzz/Depths', help='gt folder to calculate depth error')


parser.add_argument('--batch_size', type=int, default=1, help='testing batch size')
parser.add_argument('--numdepth', type=int, default=192, help='the number of depth values')

parser.add_argument('--use_mmp', help='True or False flag, input should be either "True" or "False".',
...
# input data
parser.add_argument('--outdir_down4', default='/data1/wzz/3000', help='1/4 size output dir')
parser.add_argument('--outdir_down8', default='/data1/wzz/3001', help='1/8 size output dir')
parser.add_argument('--outdir_down16', default='/data1/wzz/3002', help='1/16 size output dir')
...
def read_camera_parameters(filename, scale, flag):
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src, flag):
    ...
    mask = np.logical_and(dist < t1, relative_depth_diff < t2)
    depth_reprojected[~mask] = 0

    return mask, depth_reprojected, x2d_src, y2d_src


def filter_depth(scan_folder, out_folder, plyfilename, scale):
    # the pair file
    ...
    for ref_view, src_views in pair_data:
        ...
        ref_intrinsics, ref_extrinsics = read_camera_parameters(
            os.path.join(scan_folder, 'cams/{:0>8}_cam.txt'.format(ref_view)),scale,flag)
        ...
        for src_view in src_views:
            ...
            src_intrinsics, src_extrinsics = read_camera_parameters(
                os.path.join(scan_folder, 'cams/{:0>8}_cam.txt'.format(src_view)),scale,flag)
            ...
            geo_mask, depth_reprojected, x2d_src, y2d_src = check_geometric_consistency(ref_depth_est, ref_intrinsics,
                                                                                        ref_extrinsics,
                                                                                        src_depth_est,
                                                                                        src_intrinsics, src_extrinsics, flag)
        ...
        if (flag):
            ...
            color = ref_img[0:-16:2, 0::2, :][valid_points]  # hardcoded for DTU dataset
...
if __name__ == '__main__':
    ...
    if (args.use_mmp):
        ...
        for scan in scans:
            ...
            out_folder = os.path.join(args.outdir_down16, scan)
            # filter saved depth maps with photometric confidence maps and geometric constraints
            filter_depth(scan_folder, out_folder,
                         os.path.join(args.outdir_down16, 'mvsnet{:0>3}_l3.ply'.format(scan_id)), scale)
        ...
        for scan in scans:
            scan_id = int(scan[4:])
            scan_folder = os.path.join(args.testpath, scan)
            out_folder = os.path.join(args.outdir_down8, scan)
            # filter saved depth maps with photometric confidence maps and geometric constraints
            filter_depth(scan_folder, out_folder,
                         os.path.join(args.outdir_down8, 'mvsnet{:0>3}_l3.ply'.format(scan_id)), scale)

            # merge depth maps
        mmp.multi_scale(args.outdir_down4, args.outdir_down8, args.outdir_down16, args.outdir_down4, args.testlist)

# process down2 to calculate final point clouds
scale = 4
for scan in scans:
    scan_id = int(scan[4:])
    scan_folder = os.path.join(args.testpath, scan)
    out_folder = os.path.join(args.outdir_down4, scan)
    # step2. filter saved depth maps with photometric confidence maps and geometric constraints
    filter_depth(scan_folder, out_folder, os.path.join(args.outdir_down4, 'mvsnet{:0>3}_l3.ply'.format(scan_id)), scale)
